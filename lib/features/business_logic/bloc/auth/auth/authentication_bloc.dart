...
class AuthenticationBloc
  ...
  AuthenticationBloc({required AuthenticationUseCase authenticationUseCase})
      : authenticationsUseCase = authenticationUseCase,
        super(const AuthenticationState.firstTimeAppOpened()) {
  ...
  Future<void> _onAuthenticationStatusChanged(AuthenticationStatusChanged event,
    ...
    switch (event.status) {
      ...
      case AuthenticationStatus.firstTimeAppOpened:
        emit(const AuthenticationState.firstTimeAppOpened());
  ...
  Future<void> isLoggedIn() async {
    ...
    result.fold((l) {
      ...
      if (isLoggedIn) {
        ...
        authUser.fold((l) {
          bool isViewed =
              instance<AppSettingPreferences>().getAppOnBoardingIsViewed();
          _controller.add(isViewed
              ? AuthenticationStatus.unauthenticated
              : AuthenticationStatus.firstTimeAppOpened);
        }, (r) {
          user = r.data!;
