...
class HomeDeomScreen extends StatelessWidget {
  const HomeDeomScreen({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return const PageDeomHome();
  }
}

class PageDeomHome extends StatelessWidget {
  const PageDeomHome({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => instance<HomeBloc>()..add(GetDeomHomeData()),
      child: const HomeDeomScaffold(
        widget: Content(),
      ),
    );
  }
    ...
    return RefreshIndicator(
        child: BlocListener<HomeBloc, HomeState>(
          listener: (context, state) {
            if (state.statePage == StatePage.data) {
              if (state.isUploaded == true) {
                ...
                instance<ShowMessage>()
                    .showSnakeBar(context, AppStrings.acceptedSuccess);
              } else {
            ...
            if (state.statePage == StatePage.error) {
              if (state.failure == AppConstants.maintainFailure) {
                ...
                Navigator.pushReplacementNamed(context, Routes.updateAppRoot);
          ...
          child: OrientationBuilder(
              builder: (context, orren) => orren == Orientation.portrait
                  ? Widgets()
                  : WidgetsLandScape()),
        ),
        onRefresh: () async {});
                ...
                child: Column(
                  children: [
                    Container(
                      ...
                      height: 50,
                      child: Image.asset(ImagesAssetsManage.brandImages),
                    ),
              ...
              Container(
                ...
                child: Column(
                  children: [
                      ...
                      Column(
                        children: [
                          SizedBox(
                            ...
                            width: 300,
                            child:
                                Lottie.asset(JsonAssetManager.lottieEmptyHome),
                          ),
                          Text(
                            AppLocalizationsImpl.of(context)!
                                .translate(AppStrings.emptyContent),
                    ...
                    if (state.homeData.data.subjects.isNotEmpty)
                      Divider(
                        height: context.width * AppSize.appSize0_01,
                        endIndent: 20,
                        indent: 20,
                        thickness: 4,
            ...
            builder: (context, state) {
              return ListView.builder(
                  ...
                  itemBuilder: (context, position) {
                    return _ListItem(
                        context,
                        state.homeData.data.subjects[position],
                        state.selected ==
                            state.homeData.data.subjects[position].id);
                  });
            },
          )),
    );
    ...
    return BlocBuilder<HomeBloc, HomeState>(
      builder: (context, state) {
        return GestureDetector(
          onTap: () {
            Navigator.pushNamed(context, Routes.classSchool,
                arguments: courses);
          },
          child: Container(
            margin: const EdgeInsets.symmetric(
                horizontal: AppPadding.appPadding10,
                vertical: AppPadding.appPadding4),
            height: 60,
            width: context.width,
            child: Stack(
              children: [
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius:
                  const BorderRadius.all(Radius.circular(AppSize.appSize10)),
            ),
          ...
          return CarouselSlider(
            options: CarouselOptions(
              ...
              autoPlayAnimationDuration:
                  const Duration(milliseconds: AppValue.appValue800),
            ...
            items: state.images.map((i) {
              return CachedNetworkImage(
                ...
                errorWidget: (context, url, error) =>
                    Card(child: Image.asset(ImagesAssetsManage.splashImages)),
              );
            }).toList(),
          );
        },
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    if (state.homeData.data.subjects.isNotEmpty)
                      Divider(
                        height: context.height * AppSize.appSize0_01,
                        endIndent: 20,
                        indent: 20,
                        thickness: 4,
                        color: ColorManager.primaryColorLight,
