...
class AuthRepositoryImpl implements AuthRepository {
  ...
  AuthRepositoryImpl(
      this._remoteDataSource, this._authLocalDataSource, this._networkInfo);
    ...
    if (await _networkInfo.isConnected) {
      try {
        ...
        if (response.status == ApiInternalStatusCode.success) {
          ...
          return Left(
              ErrorMessage.getErrorAuth(response.errorId.toString()).failure);
        } else {
  ...
  @override
  Future<Either<Failure, SuccessData>> attendance(
      LessonRequest lessonRequest) async {
    if (await _networkInfo.isConnected) {
      try {
  ...
  @override
  Future<Either<Failure, SuccessData>> uploadSheet(
      SheetRequest sheetRequest) async {
    if (await _networkInfo.isConnected) {
      try {
  ...
  @override
  Future<Either<Failure, SuccessData>> addContact(
      AddContactRequest sheetRequest) async {
    if (await _networkInfo.isConnected) {
      try {
  ...
  @override
  Future<Either<Failure, MessageDataPagaination>> getContact(
      LessonRequest lessonRequest) async {
    if (await _networkInfo.isConnected) {
      try {
  ...
  @override
  Future<Either<Failure, NotePaginationOrders>> getNote(
      LessonRequest lessonRequest) async {
    if (await _networkInfo.isConnected) {
      try {
